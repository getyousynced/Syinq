      ...
      {pieces.map((p) => (
        <motion.div
          ...
          style={{
            left: `${p.left}%`,
            top: "-20px",
            width: p.shape === "rect" ? "8px" : "10px",
            height: p.shape === "rect" ? "16px" : "10px",
        ...
        <g key={index}>
          ...
          <g transform={`translate(${iconX}, ${iconY})`}>
            <foreignObject x="-12" y="-12" width="24" height="24">
              <div
                className={`w-6 h-6 ${segment.color === "#147EFB" ? "text-white" : "text-[#147EFB]"}`}
              >
  ...
  return (
    <div className="relative flex flex-col items-center">
      ...
      <motion.div
        ...
        style={{
          background:
            "radial-gradient(circle, rgba(20,126,251,0.3) 0%, transparent 70%)",
        }}
        animate={{
      ...
      >
        ...
        >
          ...
          {/* Clickable Logo Spin Trigger */}
          <foreignObject x="102" y="102" width="96" height="96">
            <motion.div
              onClick={spin}
              className={`w-full h-full flex items-center justify-center 
    rounded-full cursor-pointer overflow-hidden
    bg-black shadow-inner border-4 border-[#147EFB]
    ${isSpinning ? "pointer-events-none" : ""}`}
              whileHover={{ scale: isSpinning ? 1 : 1.08 }}
              whileTap={{ scale: isSpinning ? 1 : 0.95 }}
              animate={{
                scale: isSpinning ? 1 : [1, 1.06, 1],
              }}
              transition={{
                duration: 1.6,
        ...
        {reward && (
          ...
          >
            ...
            >
              ...
              <p className="text-lg font-semibold text-[#147EFB] mb-4">
                {reward.label}
              </p>

              <p className="text-gray-600 mb-6">
                {FUNNY_MESSAGES[reward.label]}
              </p>

              <button
                onClick={() => setReward(null)}
                className="px-6 py-3 rounded-full bg-[#147EFB] text-white font-semibold hover:bg-[#0A5BC4] transition"
              >
                Awesome 🚀
              </button>
            </motion.div>
